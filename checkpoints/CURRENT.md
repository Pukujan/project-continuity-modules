...
## Current projection — PCM-0046 amendment set frozen/merged, arms rate-gated to ~04:00Z; #122/#123 closed; decisions queued

Program state 2026-09-25 ~22:20Z: **PCM-0046 (#139)**: scorer+fixtures+rubric merged at `09638d8` (#148); arm prompt set FROZEN by amendments 11-12 (situation-only prompts, T1 format pointer, T3 intent=False) — merged at `57d5b94` (#160) with the CURRENT v1-procedure contradiction fixed; the 20 hidden arms remain blocked by the provider 429 window (probe 22:04Z: 2/2 subagent launches rejected pre-execution, retry-after ~5.9h, opens ~04:00Z 2026-09-26); zero arms run; /tmp/pcm0046-arms-v2 bundles verified unmodified (no file newer than manifest.json). **#122 / #123 CLOSED** by evidence closeout comments 5840415451 / 5840415434 (checkpoint index-freshness + sanitizer: all acceptance items evidenced; colon/cross-repo coverage at `e3c6f25` #157; dogfood proof `2063136`). **PCM-0047 (#140) delivered and CLOSED** — sanitizer + same-commit index refresh at `68765dd` (#147), closeout `1a64864` (#149), marker `626be9d` (#150). **PCM-0050 (#144)**: owner decision brief 5839017899 pending. **PCM-0048 (#142) / PCM-0049 (#143)**: owner A/B decision briefs 5839021694 / 5839021912 pending. **PCM-0028 phases 1-3 (#104/#105/#106)**: NOT released — owner direction 5830322199 requires a separate go on #100; do not start.
...
1) Run the PCM-0046 holdout per docs/plans/PCM-0046-arm-plan.md INCLUDING its 2026-09-25 amendments 1-12 (v1 prompts/bundles void; Amendment 4); the prompt set is FROZEN (Amendment 12, merged at `57d5b94` #160). Bundles already staged at /tmp/pcm0046-arms-v2 (regenerate with `python3.12 docs/plans/PCM-0046-arm-bundles.py` only if the unmodified check fails); guidance committed at docs/plans/PCM-0046-guidance-candidate.md / -baseline.md. ALL 20 arms launch only after the provider window — probes 21:15Z, 22:05Z and 22:04Z session-level all returned 429 (latest retry-after ~5.9h, opens ~04:00Z 2026-09-26); zero arms have run. TEXT arms (T2-1..5, T3-1..5, T3b-1..5) on read-only scout; WRITE arms (T1-1..5, T1b-1..5, T4-1..5) on write-capable task subagents; prompts verbatim from docs/plans/PCM-0046-arm-launcher.sh (v4). Parent stores T2/T3/T3b texts as /tmp/pcm0046-arms/results/T2-<n>.answer.txt / T3-<n>.message.txt / T3b-<n>.message.txt, then `python3.12 docs/plans/PCM-0046-arm-scorer.py capture /tmp/pcm0046-arms-v2 /tmp/pcm0046-arms/results` and `score /tmp/pcm0046-arms/results`. T3 is scored intent=False (Amendment 11; Amendment 10 superseded). Verify canonical `git status --porcelain` empty and record it. Report candidate+baseline tallies on #139, append to #137; PCM-0046 checkpoint + closeout PR; close #139; `continuity worktree remove PCM-0046`. 2) PCM-0051 (#154): DELIVERED at `e3c6f25` (#157); #154 CLOSED; #123 CLOSED by closeout 5840415434. 3) Owner decisions pending: #144 Q1-Q5 (brief 5839017899), #142 A/B (5839021694), #143 A/B (5839021912) — no SPEC/AGENTS/cli edits before answers.
