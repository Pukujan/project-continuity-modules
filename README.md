...
| issue-log-format module is 1.1.0 with diagram rules | shipped (source only) | The shipped module doc and generated guidance carry the `issue-log-format` 1.1.0 marker, adding mermaid diagram rules (draw threshold, `graph TD` default, size caps, mandatory text alternative, collapsed wide diagrams, verify-before-publish) verified against GitHub rendering ([#126](https://github.com/Pukujan/project-continuity-modules/issues/126), [matrix evidence](https://github.com/Pukujan/pcm-mermaid-matrix/issues/1)) | That the package version bumped — it stays 0.5.0 (guidance text only) — or that a published release carries the rules. Existing adopters still need the marker-replacement update | [`docs/ISSUE_LOG_FORMAT.md`](docs/ISSUE_LOG_FORMAT.md), [`src/continuity/cli.py`](https://github.com/Pukujan/project-continuity-modules/blob/0b3be9ca80da816de4621ac4e85612990084216e/src/continuity/cli.py) |
