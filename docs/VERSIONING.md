...
PCM-0058 raises the `issue-log-format` policy module to **1.2.0** (2026-09-26, [#180](https://github.com/Pukujan/project-continuity-modules/issues/180)): the module and generated guidance gain readability rules for records the tools compose — plain-word meaning before identifiers carry load, claim-first evidence with numbers as support, no unexplained acronyms or bare ids on first use, one problem sentence leading PR openings and checkpoint/CURRENT lines. Provenance: this session's own PR bodies as symptoms and the before/after exemplar in [PR #178](https://github.com/Pukujan/project-continuity-modules/pull/178#issuecomment-5842450065). The CLI/package source version stays **0.6.0** — guidance text only, no behavior contract — and no package is published; the protocol remains **0.1.0-draft**. Existing adopters follow the marker-replacement update path in `docs/ISSUE_LOG_FORMAT.md`.
